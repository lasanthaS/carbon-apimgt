...
public class APIKeyMgtRemoteUserStoreMgtService extends AbstractAdmin {
    ...
    public String[] getUserRoles(String username) throws APIManagementException {
        ...
        try {
            UserManagerHolder.getUserManager().getRoleListOfUser(MultitenantUtils.getTenantAwareUsername(username));
        } catch (UserException e) {
    ...
    public BasicAuthValidationInfoDTO getUserAuthenticationInfo(String username, String password)
        ...
        try {
            isAuthenticated = UserManagerHolder.getUserManager()
                    .authenticate(MultitenantUtils.getTenantAwareUsername(username), password);
            if (isAuthenticated) {
                basicAuthValidationInfoDTO.setAuthenticated(true);
                domainQualifiedUsername = UserManagerHolder.getUserManager()
                        .addDomainToName(username, UserManagerHolder.getUserManager().getDomainFromThreadLocal());
            ...
            userRoles = UserManagerHolder.getUserManager()
